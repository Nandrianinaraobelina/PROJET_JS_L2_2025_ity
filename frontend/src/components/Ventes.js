...
function Ventes({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log(
        "🎬 Création des ventes et achats pour",
      ...
      console.log(
        `📊 Résultats: ${successes.length} succès, ${failures.length} échecs`
      );

      if (failures.length > 0) {
        console.error("❌ Échecs détectés:", failures);
        throw new Error(
          `${failures.length} transaction(s) ont échoué. Vérifiez la console pour plus de détails.`
        );
      }
      ...
      successNotification.className =
        "notification-toast-modern notification-success-modern";
    ...
    <div className="section-card fade-in">
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <div className="d-flex gap-2 flex-wrap">
          <button
            className="btn btn-primary-modern btn-glass-modern"
            onClick={() => {
              if (setActiveTab) {
          ...
          <button
            className="btn btn-outline-primary btn-ghost-modern"
            onClick={() => {
              if (setActiveTab) {
                setActiveTab("catalogue");
              }
          ...
          {selectedFilms.length > 0 && (
            <>
              <button
                className="btn btn-danger-modern btn-glass-modern"
                onClick={() => {
                  if (
                    window.confirm(
                      "Voulez-vous vraiment vider toute la sélection ?"
                    )
                  ) {
                    selectedFilms.forEach(
                      (film) => onFilmRemoved && onFilmRemoved(film.ID_PROD)
                    );
                    setForm({ ID_CLIENT: "", ID_VENDEUR: "", DateVente: "" });
                  }
                }}
                      ...
                      <div className="d-flex justify-content-between align-items-start">
                        <div>
                          ...
                          <div className="d-flex align-items-center">
                            <label className="form-label me-2 mb-0 small">
                              Quantité:
                            </label>
                            <input
                              type="number"
                        ...
                        <button
                          className="btn btn-sm btn-danger-modern btn-glass-modern"
      ...
      {selectedFilms.length === 0 && (
        <div className="alert alert-info text-center mb-4">
          ...
          <p className="mb-2">
            Retournez au catalogue pour choisir des films à acheter.
          </p>
          ...
          <div className="d-flex align-items-center">
            ...
            <div>
              ...
              <ol className="mb-0 mt-2">
                ...
                <li>
                  Miverena eto rehefa aveo amaetrahana ny anaranao sy date ary
                  andray anao teto
                </li>
      ...
      <form className="row g-3 mb-4 fade-in" onSubmit={handleSubmit}>
        ...
        <div className="col-md-4">
          ...
          <select
            className="form-control"
            name="ID_VENDEUR"
            value={form.ID_VENDEUR}
            onChange={handleChange}
            required
          >
            <option value="">Choisir un vendeur</option>
            {vendors.map((v) => (
              <option key={v.ID_VENDEUR} value={v.ID_VENDEUR}>
                {v.NomVendeur} {v.PrenomVendeur}
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="col-12">
          <button
            type="submit"
            className="btn btn-success-modern btn-glass-modern btn-lg"
            disabled={formLoading || selectedFilms.length === 0}
          >
            {formLoading ? (
              <>
                <span
              ...
              <>
                ...
                <small className="text-white-50">
                  ...
                  {selectedFilms.length > 1 ? "s" : ""} → Facturation)
                </small>
              </>
            )}
          </button>
