...
module.exports = (db) => {
  ...
  router.post('/', [
    ...
    body('EmailCli').optional().isEmail().withMessage('Format d\'email invalide'),
    body('TelephoneCli').optional().isLength({ min: 1 }).withMessage('Téléphone invalide'),
  ], (req, res) => {
    ...
    const sql = 'INSERT INTO CLIENT (NomCli, PrenomCli, EmailCli, TelephoneCli, AdresseCli, Ville, Pays) VALUES (?, ?, ?, ?, ?, ?, ?)';
    const values = [
      NomCli,
      PrenomCli,
      EmailCli || null,
  ...
  router.put('/:id', [
    ...
    console.log('🔄 Mise à jour client ID:', req.params.id, 'avec:', { NomCli, PrenomCli, EmailCli, TelephoneCli });

    const sql = 'UPDATE CLIENT SET NomCli=?, PrenomCli=?, EmailCli=?, TelephoneCli=? WHERE ID_CLIENT=?';
    const values = [
      NomCli,
      PrenomCli,
      EmailCli || null,
