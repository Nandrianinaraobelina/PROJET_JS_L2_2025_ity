...
module.exports = (db) => {
  ...
  router.post(
    ...
    (req, res) => {
      ...
      db.query(sql, [ID_PROD, ID_VENDEUR, DateVente], (err, result) => {
        ...
        res
          .status(201)
          .json({ id: result.insertId, ID_PROD, ID_VENDEUR, DateVente });
      });
    }
  );
  ...
  router.get("/:id", (req, res) => {
    db.query(
      "SELECT * FROM VENTE_PRODUIT WHERE ID_VENTE = ?",
      [req.params.id],
      (err, results) => {
        if (err) return res.status(500).json({ error: err });
        if (results.length === 0)
  ...
  router.put(
    ...
    (req, res) => {
      ...
      const { ID_PROD, ID_VENDEUR, DateVente } = req.body;
      const sql =
        "UPDATE VENTE_PRODUIT SET ID_PROD=?, ID_VENDEUR=?, DateVente=? WHERE ID_VENTE=?";
      db.query(
        sql,
        [ID_PROD, ID_VENDEUR, DateVente, req.params.id],
        (err, result) => {
          if (err) return res.status(500).json({ error: err });
  ...
  router.delete("/:id", authenticateToken, (req, res) => {
    db.query(
      "DELETE FROM VENTE_PRODUIT WHERE ID_VENTE = ?",
      [req.params.id],
      (err, result) => {
        if (err) return res.status(500).json({ error: err });
